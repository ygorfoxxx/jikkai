...
// Caminho: /api/discord-dm
// Recomendado: configurar DISCORD_BOT_TOKEN nas Environment Variables da Vercel.
...
module.exports = async function handler(req, res) {
  // Mesmo domínio na Vercel normalmente não precisa CORS, mas deixo seguro para testes.
  ...
  try {
    ...
    // Preferência: variável de ambiente. Fallback: token enviado pelo portal, se você optar por manter assim.
    ...
    if (!/^\d{15,25}$/.test(discordId)) return json(res, 400, { ok: false, error: "Discord ID inválido" });
