...
function applyOverlayMode(mode = "hud", section = overlaySection) {
  ...
  overlay.setMinimumSize(overlayMode === "panel" ? 560 : 260, overlayMode === "panel" ? 560 : 150);
  overlay.setBounds(overlayBounds(overlayLayoutMode), false);
  overlay.setOpacity(overlayPrefs(overlayLayoutMode).opacity || 0.92);
...
app.on("second-instance", () => {
  showOverlayPanel(desktopPrefs.overlay?.lastSection || "missao");
});
...
app.whenReady().then(async () => {
  ...
  registerShortcuts();
  maybeStartNativeOverlayHost();
  if (START_PANEL) setTimeout(() => showOverlayPanel(desktopPrefs.overlay?.lastSection || "missao"), 350);
